...
## Dev → Build → Deploy Reference

> **Rule: `dist/` is generated. Never commit it.**
> Build artifacts live only in CI and Azure — not in source control.
...
```bash
npm run ci           # type-check → lint → build (replicates the CI gate)
```

A failed `npm run ci` locally means CI will also fail — fix it before pushing.

### What CI does on every push

1. `npm run type-check` — TypeScript strict mode, no type errors allowed
2. `npm run lint` — ESLint with hooks + refresh rules
3. `npm run build` — `tsc -b && vite build` → outputs to `dist/`
4. Azure Static Web Apps uploads `dist/` to CDN — `dist/` is **never tracked in git**

### Telemetry events generated by the sync pipeline

Every button click in the Intelligence Sync panel emits structured telemetry visible in the browser console:

| Action | Event |
|---|---|
| Rebuild Day | `sync_started` → `sync_completed` / `sync_failed` |
| Match Screenshots | `screenshot_match_started` → `screenshot_match_completed` |
| Add Expense | `manual_expense_added` |
| Upload Screenshots | `uber_cards_uploaded` |
| API failure | `api_failure` (with endpoint + status code) |

These events are the observability contract between the dashboard and SummitOS Intelligence.

### Environment variables

Copy `.env.template` → `.env.local` for local overrides.
**Never commit `.env` or `.env.local`** — they are in `.gitignore`.
Production values are set in Azure Static Web Apps → Settings → Configuration.
